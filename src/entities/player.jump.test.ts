...
describe('reactHit — 피해 없이 피격 연출(튜토리얼 무피해)', () => {
  it('HP 불변 + 무적 점멸 + hit 애니, 무적 해제 후 run 복귀', () => {
    const p = new Player();
    const hp0 = p.hp;
    p.reactHit();
    step(p, 1);
    expect(p.hp).toBe(hp0); // 피해 없음
    expect(p.invulnerable).toBe(true);
    expect(p.animState).toBe('hit'); // 피격 연출
    step(p, 40); // 0.66s > hitInvuln(0.5)
    expect(p.animState).toBe('run');
    expect(p.hp).toBe(hp0);
  });

  it('이미 무적이면 재트리거 안 함(중복 방지)', () => {
    const p = new Player();
    p.takeDamage(5); // 무적 + HP 감소
    const hpAfter = p.hp;
    p.reactHit(); // 무적 중 → no-op
